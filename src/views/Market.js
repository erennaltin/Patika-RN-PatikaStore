...
const Market = props => {
  ...
  useEffect(() => {
    function setList() {
      if (props.search === '') {
        setFilteredList([...elements]);
      } else {
        const filtered = elements.filter(element =>
          element.title.toLowerCase().search(props.search.toLowerCase()) !== -1
            ? true
            : false,
        );
    ...
    setList();
  }, [setFilteredList, props.search, elements]);

  const RenderItem = ({item}) => <MarketElement element={item} />;
